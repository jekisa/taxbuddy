...
import { effectiveSubscriptionExpiresAt, subscriptionAccess } from "./billing";
import { getEnv, isProduction } from "./env";

const uri = getEnv("MONGODB_URI");
const dbName = getEnv("MONGODB_DB", "taxbuddy");
const sessionSecret = getEnv("SESSION_SECRET", "taxbuddy-dev-session-secret");
...
export async function ensureIndexes(db) {
  await Promise.all([
    ...
    db.collection("workspaces").createIndex({ updatedAt: 1 }),
    db.collection("document_archives").createIndex({ userId: 1, updatedAt: -1 }),
    db.collection("document_archives").createIndex({ userId: 1, module: 1, updatedAt: -1 }),
